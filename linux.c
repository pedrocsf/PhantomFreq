...
// (Inclua os mesmos headers e mantenha as outras funções como antes)
...
int main()
{
    ...
    // Isso requer privilégios de root para funcionar (execute com sudo).
    if (hci_dev_down(dev_id) < 0)
    {
        perror("Falha ao desativar o dispositivo HCI");
        // Não saímos, pois pode já estar 'down'. Tentamos continuar.
    }
    ...
    while (1)
    {
        printf("Broadcasting as %s with MAC %s\n", names[idx], macs_str[idx]);
        ...
        printf("Advertising... verifique em um scanner BLE.\n");
